...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen, onClose }) => {
  ...
  const { channelHash, isConnected } = useChat();
    ...
    <div className="settings-scrim" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside className="settings-panel" aria-label={viewTitles[view]}>
        ...
        <div className="settings-panel__body">
          {view === 'settings' && (
            <>
              <div className="identity-card">
                ...
                <div><strong>Your space</strong><p>Identity setup is not available yet.</p></div>
          ...
          {view === 'privacy' && (
            <section className="settings-detail">
              <div className="detail-intro"><h3>Designed around your privacy</h3><p>Your current conversation uses its private invitation to protect messages in transit.</p></div>
              <div className="state-card state-card--positive"><ShieldIcon size={21} /><div><strong>Conversation protection is on</strong><p>Sensitive invitation details stay in the link fragment and are not sent as a normal page request.</p></div></div>
          ...
          {view === 'verification' && (
            <section className="settings-detail verification-view">
              ...
              <div className="unavailable-card">
                <StatusPill tone="quiet">Not available yet</StatusPill>
                <strong>Safety code coming later</strong>
                <p>Persistent identity exists in the security prototype but is not connected to live conversations yet. No code is shown until it can be genuine.</p>
              </div>
